...
/// %LOCALAPPDATA%/Rivet/models. GGML is the only format whisper.cpp reads (ADR 0001).
///
/// Downloads straight from the ggml-org model repository rather than through Whisper.net's
/// bundled downloader: that one hands back a non-seekable stream with no length, so a progress
/// bar over it can only spin. A ~1.5 GB download with no visible progress reads as a hang.
/// </summary>
public sealed class WhisperModelProvider : IWhisperModelProvider
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        // gigabytes into memory. It is also what makes Content-Length available up front.
    ...
    /// <summary>File names as published by ggml-org/whisper.cpp.</summary>
