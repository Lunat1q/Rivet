...
{
    /// <summary>e.g. "0.1.0". The build stamps this; a dev build reports whatever the csproj says.</summary>
    public static string Current { get; } = Read();

    private static string Read()
    {
        // This assembly, not the entry assembly: under a test host — or anything else that loads
        // Rivet.App — the entry assembly is the host, and we would report *its* version.
        var assembly = typeof(AppVersion).Assembly;

        // InformationalVersion carries the git hash too ("0.1.0+a97ec97…"). Drop it: the number is
