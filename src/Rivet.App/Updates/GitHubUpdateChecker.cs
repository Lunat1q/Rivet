...
{
    private const string LatestRelease = "https://api.github.com/repos/Lunat1q/Rivet/releases/latest";
    ...
    public async Task<UpdateInfo?> CheckAsync(CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            // The Inno Setup installer. Named Rivet-Setup-*.exe by installer/Rivet.iss.
            var installer = release.Assets.FirstOrDefault(
                a => a.Name.EndsWith(".exe", StringComparison.OrdinalIgnoreCase));

            if (installer is null)
            {
                _logger.LogWarning("Release {Tag} has no .exe asset; ignoring it", release.TagName);
                return null;
            }
